...
load("./0BC_data_BCAC/BCAC_iCOGS_2020.rdata")
...
for (i in expfile) {
  ...
  if (is.null(out)) {
    ...
    outSNP <- BCAC_iCOGS_2020$SNP[BCAC_iCOGS_2020$pval.outcome<1e-5] 
    ...
    if (nrow(dat) == 0) {
      ...
      if (res$pval>0.05 | res$pval=='NaN') {
        ...
        if (OR$method == 'Inverse variance weighted') {
          ...
          save(exp,out,dat,res,OR,file = paste0('./2.5metabQTL_BCAC_iCOGS_MR/',trait,
                                                "_mrResult.rdata"))
          AllOR <- rbind(AllOR,OR)

          single <- mr_leaveoneout(dat)
          mr_leaveoneout_plot(single)
          ggsave(filename = paste0('./2.5metabQTL_BCAC_iCOGS_MR/',trait,'_leaveoneout_plot.pdf'),width = 6,height = 5)
          

          mr_scatter_plot(res,dat)
          ggsave(filename = paste0('./2.5metabQTL_BCAC_iCOGS_MR/',trait,'_mr_scatter_plot.pdf'),width = 6,height = 5)
          

          res_single <- mr_singlesnp(dat,all_method = c("mr_ivw")) 
          mr_forest_plot(res_single)
          ggsave(filename = paste0('./2.5metabQTL_BCAC_iCOGS_MR/',trait,'_mr_forest_plot.pdf'),width = 5,height = 6)
          

          mr_funnel_plot(res_single)
          ggsave(filename = paste0('./2.5metabQTL_BCAC_iCOGS_MR/',trait,'_mr_funnel_plot2.pdf'),width = 6,height = 4.5)
...
save(AllOR,file = '2.5all_metabQTL_mrres_BCAC_iCOGS.rdata')
write.csv(AllOR,file = '2.5all_metabQTL_mrres_BCAC_iCOGS.csv')
...
save(AllOR_NOhete_FDR,file = '2.5all_metabQTL_mrres_BCAC_iCOGS_NOhete_FDR.rdata')
write.csv(AllOR_NOhete_FDR,file = '2.5all_metabQTL_mrres_BCAC_iCOGS_NOhete_FDR.csv')
...
save(AllOR_NOhete_BFR,file = '2.5all_metabQTL_mrres_BCAC_iCOGS_NOhete_BFR.rdata')
write.csv(AllOR_NOhete_BFR,file = '2.5all_metabQTL_mrres_BCAC_iCOGS_NOhete_BFR.csv')
...
ggplot(AllOR,aes(x = b, y = -log10(FDR),color=Effect)) +
  ...
  labs(x = "Beta (effect size)", 
       ...
       title = 'All MR results between metabolites and BC risk (BCAC_iCOGS)')+
  ggrepel::geom_label_repel(aes(label = label),size = 3,
                            color="black",box.padding = unit(0.4, "lines"), 
                            segment.color = "black",
                            segment.size = 0.4)
ggsave(filename = '2.5volcano_plot_BCAC_iCOGS.pdf',width = 6.5,height = 4.5) 
